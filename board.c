...
board_copy(struct board *b2, struct board *b1)
{
	...
	void *x = malloc(bsize + gsize + fsize + nsize);
	memcpy(x, b1->b, bsize + gsize + fsize + nsize);
	b2->b = x; x += bsize;
	b2->g = x; x += gsize;
	b2->f = x; x += fsize;
	b2->n = x; x += nsize;
...
\
		...
		int fsize = ((b2))->size * (b2)->size * sizeof(*(b2)->f); \
		int nsize = ((b2))->size * (b2)->size * sizeof(*(b2)->n); \
		void *x = alloca(bsize + gsize + fsize + nsize); \
		memcpy(x, b1->b, bsize + gsize + fsize + nsize); \
		((b2))->b = x; x += bsize; \
		((b2))->g = x; x += gsize; \
		((b2))->f = x; x += fsize; \
...
board_play_f(struct board *board, struct move *m, int f)
{
	...
	if (unlikely(!gid)) {
		...
		void *b = board->b, *g = board->g, *f = board->f, *n = board->n, *gi = board->gi;
		...
		board->b = b; board->g = g; board->f = f; board->n = n; board->gi = gi;
